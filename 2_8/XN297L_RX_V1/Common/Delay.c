...
{
    T3CON = 0x03; //Timer3 Clock = Fsys/8
    set_TR3;      //Trigger Timer3
    while (u32CNT != 0)
    {
        RL3 = LOBYTE(TIMER_DIV8_VALUE_10us); //Find  define in "Function_define.h" "TIMER VALUE"
        RH3 = HIBYTE(TIMER_DIV8_VALUE_10us);
        while ((T3CON & SET_BIT4) != SET_BIT4)
            ; //Check Timer3 Time-Out Flag
        clr_TF3;
        u32CNT--;
...
void Timer3_Delay1ms(UINT32 u32CNT)
{
    ...
    while (u32CNT != 0)
    {
        RL3 = LOBYTE(TIMER_DIV16_VALUE_1ms); //Find  define in "Function_define.h" "TIMER VALUE"
        RH3 = HIBYTE(TIMER_DIV16_VALUE_1ms);
        while ((T3CON & SET_BIT4) != SET_BIT4)
            ; //Check Timer3 Time-Out Flag
        clr_TF3;
        u32CNT--;
