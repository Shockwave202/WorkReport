...
uint8_t motor_speed_regulation_rate;
...
{ //The startup time is approximate to 1.6S.
  ...
  joystick_x_level = MID_LEVEL; //Default settings,
...
void loop()
{
  ...
  if (joystick_sampling_rate == 100) //Approximate to 1ms(100).
  {
    joystick_sampling_rate = 0;
    /*----Toggle D4_BOARD-----
 ...
 //Simple version of joystick ctrlled vehicle.
  ...
  motor_speed_regulation_rate++;
  if (motor_speed_regulation_rate == 130) //Approximate to 1ms
  {
    motor_speed_regulation_rate = 0;
    /*----Toggle D4_BOARD-----
    if (digitalRead(D4_BOARD))
